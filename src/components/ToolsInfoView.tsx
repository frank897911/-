...
export const ToolsInfoView: React.FC<ToolsInfoViewProps> = ({
  ...
  return (
    <div className="space-y-4 pb-20">
      ...
      <div className="grid grid-cols-4 gap-1.5 p-1 bg-white rounded-2xl border border-[#F1E9DB] shadow-2xs">
        {[
          ...
          return (
            <button
              ...
              className={`flex flex-col sm:flex-row items-center justify-center gap-1 py-2 px-1 rounded-xl text-xs font-semibold transition-all ${
                isActive
      ...
      {subTab === 'flights' && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          ...
          {nextFlight && (
            <div className="bg-white p-4 rounded-2xl border border-[#F1E9DB] shadow-2xs flex items-center justify-between">
              <div>
                <span className="text-[11px] font-medium text-[#2B7A82] bg-[#A5DEE4]/20 px-2.5 py-0.5 rounded-full border border-[#A5DEE4]/40">
                  距離班機起飛時間 ✈️
                </span>
          ...
          {flights.map((flight) => (
            <div
              key={flight.id}
              className="bg-white p-4 rounded-2xl border border-[#F1E9DB] shadow-2xs space-y-3"
            >
              <div className="flex items-center justify-between border-b border-[#F1E9DB] pb-2.5">
                <div className="flex items-center gap-2">
                  <span className="p-1.5 bg-[#FFF9F2] text-[#D45068] rounded-xl border border-[#F1E9DB]">
              ...
              {/* Route & Times */}
              <div className="grid grid-cols-3 gap-2 text-center bg-[#FFF9F2] p-3 rounded-xl border border-[#F1E9DB]">
                <div>
              ...
              <div className="flex items-center justify-between text-xs text-[#5C554E] bg-white p-2.5 rounded-xl border border-[#F1E9DB]">
          ...
          {hotels.map((hotel) => (
            ...
            >
              <div className="flex items-start justify-between border-b border-[#F1E9DB] pb-2.5">
                ...
                <StampSticker type="done" text="已預訂" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs bg-[#FFF9F2] p-2.5 rounded-xl border border-[#F1E9DB]">
                <div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-[#F1E9DB]">
                {hotel.mapCode && (
                  <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 bg-white border border-[#A5DEE4]/60 text-[#2B7A82] rounded-xl text-xs font-mono font-medium"
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-3 py-1.5 bg-[#4A4A4A] hover:bg-[#333333] text-white rounded-xl text-xs font-semibold shadow-xs ml-auto"
                >
      ...
      {subTab === 'emergency' && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          <div className="bg-[#FFF9F2] p-3.5 rounded-2xl border border-[#F1E9DB] flex items-center gap-2 text-xs text-[#4A4A4A]">
            <ShieldAlert className="w-5 h-5 text-[#D45068] flex-shrink-0" />
            <span>在日本遇到交通事故、意外突發狀況或護照遺失時，請隨時點擊一鍵撥打或複製聯絡資訊！</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {emergencyContacts.map((contact) => (
              <div
                key={contact.id}
                className="bg-white p-3.5 rounded-2xl border border-[#F1E9DB] shadow-2xs flex flex-col justify-between space-y-2"
              >
                <div>
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-bold text-[#4A4A4A]">{contact.title}</h4>
                    <span className="text-[11px] font-semibold text-[#D45068] bg-[#F8C3CD]/20 px-2 py-0.5 rounded-full border border-[#F8C3CD]/40">
                      {contact.phone}
                    </span>
                  </div>
                  ...
                  {contact.tips && (
                    <p className="text-[10px] text-[#8C827A] mt-1 italic">💡 {contact.tips}</p>
                  )}
                </div>
                ...
                <div className="flex items-center justify-end gap-1.5 pt-2 border-t border-[#F1E9DB]">
                  ...
                  <a
                    ...
                    className="flex items-center gap-1 px-3 py-1 bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg text-[11px] font-semibold transition-colors"
                  >
      ...
      {subTab === 'budget' && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          ...
          <div className="bg-white p-4 rounded-2xl border border-[#F1E9DB] shadow-2xs space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-xs font-medium text-[#8C827A]">日本旅程總預算與記帳統計</span>
              ...
              <button
                ...
                className="flex items-center gap-1 px-3.5 py-1.5 bg-[#4A4A4A] hover:bg-[#333333] text-white rounded-xl text-xs font-semibold shadow-xs active:scale-95"
              >
          ...
          {isAddingExpense && (
            <motion.form
              ...
              className="bg-[#FFF9F2] p-4 rounded-2xl border border-[#F1E9DB] shadow-xs space-y-3 text-xs"
            >
              <h4 className="font-bold text-[#9E6B00] text-sm flex items-center gap-1">
                <Plus className="w-4 h-4" /> 記錄一筆新的花費
              </h4>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-medium text-[#4A4A4A] mb-1">花費項目名稱</label>
                  <input
                    type="text"
                    required
                    placeholder="例：敘敘苑燒肉、Suica加值"
                    value={expTitle}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  >
                    ...
                    <option value="ticket">🎟️ 門票票券</option>
                    <option value="other">🏷️ 其他雜支</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="credit_card">💳 信用卡刷卡</option>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-white text-[#5C554E] rounded-xl border border-[#F1E9DB]"
                >
                  取消
                </button>
                <button
          ...
          <div className="space-y-2">
            {expenses.map((item) => (
              <div
                key={item.id}
                className="bg-white p-3 rounded-2xl border border-[#F1E9DB] shadow-2xs flex items-center justify-between text-xs"
              >
                <div>
